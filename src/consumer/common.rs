...
use crate::{common::{record::{RecordStream, Offset, PartitionOffset}, topic::{Partition, TopicPartition, TopicPartitionList, TopicPartitionOffsetMap}}, error::ConsumerError};
...
pub trait AsyncConsumer<K, V>: Drop {
    ...
    async fn commit(&self, commit: ConsumerCommit) -> Result<TopicPartitionOffsetMap, ConsumerError>;
...
impl ConsumerCommit {
    ...
    fn with_timeout(self, timeout: Duration) -> Self {
        ConsumerCommit{ timeout: Some(timeout), bounds: self.bounds }
    }

    fn with_topics(self, bounds: HashSet<String>) -> Self {
        ConsumerCommit { timeout: self.timeout, bounds: CommitBounds::Topics(bounds) }
    }

    fn with_partitions(self, bounds: TopicPartitionList) -> Self {
        ConsumerCommit { timeout: self.timeout, bounds: CommitBounds::Partitions(bounds) }
    }

    fn with_offsets(self, bounds: TopicPartitionOffsetMap) -> Self {
        ConsumerCommit { timeout: self.timeout, bounds: CommitBounds::Offsets(bounds) }
    }
}
...
impl ConsumerSeek {
    fn new(topic_partition: TopicPartition, offset: PartitionOffset) -> Self {
        ConsumerSeek { topic_partition, offset, metadata: None }
    }
